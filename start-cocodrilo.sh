...
dotnet run --project src/CocodriloStore.Web --urls="http://localhost:$MVC_PORT" > logs-mvc.log 2>&1 &

echo "🚀 Iniciando CocodriloStore API na porta $API_PORT..."
dotnet run --project src/CocodriloStore.Api --urls="http://localhost:$API_PORT" > logs-api.log 2>&1 &

sleep 1
echo "✅ Ambos os projetos foram iniciados!"
echo "🌐 MVC:  http://localhost:$MVC_PORT"
